...
Once your above configurations are set up, you are ready to deploy your site! Be sure to sign up for a [Netlify](https://app.netlify.com/signup) account if you haven’t already and log in to it. Click the “New site from Git” button and give access to the repository you created in the previous step. Your build settings are automatically filled out for you from the `netlify.toml` in the template. To enter your environment variables, click “Show advanced” then “New variable and fill in the k*ey* input as **CHEC_PUBLIC_KEY** and the *value* input with your **Public Key.** You can access your API key in your Chec dashboard under Setup, then navigate to the Developer tab to copy your Public Key. The value is automatically encrypted and stored in Netlify’s system.
...
*From here on out, whenever code blocks are referenced, we are intentionally leaving out code lines that do not relate to the sections we are currently focusing on. We will cascade the code in as we go through the guide.*
